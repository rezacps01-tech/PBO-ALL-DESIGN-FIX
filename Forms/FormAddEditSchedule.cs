...
namespace AgroGrandong.Forms
{
    public partial class FormAddEditSchedule : BaseForm
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            var s = new Schedule {
                ScheduleId = _edit?.ScheduleId ?? 0, Location = txtLocation.Text.Trim(),
                Address = txtAddress.Text.Trim(), ScheduleDate = dtpDate.Value.Date,
                StartTime = dtpStartTime.Value.TimeOfDay, EndTime = dtpEndTime.Value.TimeOfDay,
                Capacity = cap, PricePerKg = price, Status = cmbStatus.SelectedItem?.ToString() ?? "available",
                Notes = txtNotes.Text.Trim(), CreatedBy = _user.UserId
            };
